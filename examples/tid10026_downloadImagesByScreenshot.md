...
<template tid="10026" version="0.0.6" commited="true">
  <attrs>
  ...
  <actions>
    <!-- Solution 3 - Capture screenshot and save them(not limited to images) -->
    <action_goto url="${inParas.para1}" wait="5000" />
    <action_scroll_by maxtimes="2" interval="3000" />
    <action_loopineles maxloops="5">
      ...
      <action_setvar_templstr varname="filename">
        <templstr templ="${imageUrl}" />
        <transform>
          <fun_urlget attrorpath="path_name" extname="png" />
        </transform>
      </action_setvar_templstr>
      <action_setvar_get varname="imageContent">
        <get_screenshot path="${filename}">
          <element loc="." />
        </get_screenshot>
      </action_setvar_get>
    </action_loopineles>

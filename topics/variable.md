...
- _varname_: Specifies the variable name through the varname attribute, and the corresponding value will be assigned to that variable.
- _setkeys_: Similar to object destructuring, assigns the values ​​of certain keys of an object to the corresponding variables.
...
The input parameters of the completed task are read-only, such as "origParas.para1", which are only valid in [check template](/topics/check).
